...
const page = () => {
    ...
    useEffect(() => {
        ...
        supabase.auth.getUser().then((res) => {
            ...
            supabase.from('Profile').select('first_name,last_name,bio').eq('id',res.data.user.id).limit(1).single().then((data) => {
                    ...
                    <Card className={`bg-dark rounded border-0 max-w-[400px]`}>
                        ...
                        <CardContent className={`grid grid-rows-2 gap-3`}>
                            <div className={`flex flex-col gap-2`}>
                                ...
                                <ErrorMessage name='first_name' render={(msg) => <div className={`text-accent2 text-sm`}>{msg}</div>} />
                            </div>
                            <div className={`flex flex-col gap-2`}>
                                <label htmlFor="last_name">Last name</label>
                                <Field name='last_name' placeholder={lastName} className={`p-[10px] text-dark max-w-[400px]`}/>
                                <ErrorMessage name='last_name' render={(msg) => <div className={`text-accent2 text-sm`}>{msg}</div>} />
                            </div>
                            <div className={`flex flex-col gap-2`}>
